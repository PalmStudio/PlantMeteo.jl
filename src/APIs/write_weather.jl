"""
    write_weather(file, weather; vars=setdiff(propertynames(weather), ATMOSPHERE_COMPUTED), duration=Dates.Minute)

Write a PlantMeteo weather table to disk.

Use this after standardizing a local file, downloading weather from an API, or preparing a cleaned
weather table that should be reused later. By default, variables that can be recomputed from the
core atmospheric inputs are omitted from the exported file.

"""
    to_daily(weather, args...)

Aggregate sub-daily weather to one row per civil day.

Use `to_daily` when the source weather is hourly or sub-hourly and the model expects a standard
daily weather table. The function applies sensible defaults for common variables: it averages
state-like variables, sums precipitation, computes `Tmin` and `Tmax`, and integrates radiation
fluxes over the day.
...
daily = to_daily(hourly, :T => mean => :Tmean)
```
"""
...
function default_transformation(df)
    ...
    add_transformations!(df, trans, (:duration,), sum_duration; error_missing=false)
...
"""
function sum_duration(x::T) where {T<:AbstractVector{<:Dates.Period}}
    return Dates.Day(sum(x))
end

function sum_duration(x::T) where {T<:AbstractVector{<:Dates.CompoundPeriod}}
    return Dates.Day(sum(Dates.periods(sum(x))))
end
